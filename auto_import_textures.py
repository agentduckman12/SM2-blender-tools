bl_info = {
    ...
    "version": (3, 5),
    "author": "violet :3",
    "description": "Import textures into SM2 Universal Shader for selected or all materials. Handles _cc logic and 'base color / _a' node. Appends node groups from blend if missing."
}
...
GROUP_BLEND_NAME = "SM2 MP Shader.blend"
...
def append_node_groups_if_missing():
    if any(ng.name.startswith(GROUP_PREFIX) for ng in bpy.data.node_groups):
        return

    addon_dir = os.path.dirname(os.path.abspath(__file__))
    blend_path = os.path.join(addon_dir, GROUP_BLEND_NAME)
    if not os.path.isfile(blend_path):
        print(f"Error: Required blend file not found: {blend_path}")
        return

    with bpy.data.libraries.load(blend_path, link=False) as (data_from, data_to):
        node_groups = [ng for ng in data_from.node_groups if ng.startswith(GROUP_PREFIX)]
        data_to.node_groups = node_groups
        print(f"Appended node groups: {node_groups}")


def import_textures_to_material(mat, files, use_cc):
    append_node_groups_if_missing()
    ...
    for idx, (key, p) in enumerate(paths.items()):
        ...
        tex.image = img
        tex.label = key
        tex.name = key if key != 'basecolor' else 'basecolor'
